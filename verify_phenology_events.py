...
from run_phenology_events import (
    ROOT, SOURCE, OUTPUT, CONFIG, CODE, IDENTITY, local, read_json, digest, check_hashes,
...
def independent_imports():
    ...
    tree = ast.parse(local("wp_core/phenology_events.py").read_text(encoding="utf-8"))
...
if __name__ == "__main__":
    result = verify()
    target = ROOT / "server_data/review/events_20260906_v1/verification.json"
    if target.exists():

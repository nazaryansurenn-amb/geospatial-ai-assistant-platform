...
const INITIAL_LAND_MODE = new URLSearchParams(window.location.search).get("view") === "use-type"
  ? "land_use_type"
  : "activity_2026";
...
function parcelPopupContent(parcel) {
  ...
  if (parcel.activity?.roadExcluded) {
    ...
    historyClass.textContent = HISTORY_CLASS_LABELS[parcel.history.historyClass];
...
export default function App() {
  ...
  }, [activityPreviewVisible, cropTypeStatus, historyStatus, landMode, landScope, mapReady]);
        ...
        {layerPanelOpen && (
          <div className="layer-panel-body">
            ...
            <section className="layer-group" aria-labelledby="land-layers-title">
                  ...
                  {activityStatus === "ready" && activitySummary && (
                    <dl className="activity-summary">
                      <div>
                        ...
                        <dd>{formatCount(activitySummary.open_field_parcel_count)}</dd>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <dd>{formatCount(activitySummary.activity_review_parcel_count)}</dd>
                      </div>
                    </dl>
                  )}
                </div>
                  ...
                  {historyStatus === "ready" && historySummary && (
                    <dl className="activity-summary">
                      <div>
                        ...
                        <dd>{formatCount(historySummary.eligible_parcel_count)}</dd>
                      </div>
                      <div>
                        <dt>EO մշակված հողամասեր</dt>
                        <dd>{formatCount(historySummary.processed_parcel_count)}</dd>
                      </div>
                      <div>
                        <dt>Ավտոմատ դասակարգված</dt>
                        <dd>{formatCount(historySummary.automatic_classified_count)}</dd>
                      </div>
                      <div>
                        <dt>Պահանջվում է լրացուցիչ ստուգում</dt>
                        <dd>{formatCount(historySummary.review_count)}</dd>
                      </div>
                    </dl>
                  )}
                </div>
                ...
                <div className="activity-preview" aria-label="Հողօգտագործման տեսակի նախնական դասակարգում">
                  ...
                  {cropTypeStatus === "ready" && cropTypeSummary && (
                    <dl className="activity-summary">
                      ...
                      <div>
                        <dt>Վերլուծվող հողամասեր</dt>
                        <dd>{formatCount(cropTypeSummary.eligible_parcel_count)}</dd>
                      </div>
                      <div>
                        <dt>Միամյա մշակաբույսեր</dt>
                        <dd>{formatCount(cropTypeSummary.class_counts.annual)}</dd>
                      </div>
                      <div>
                        <dt>Բազմամյա մշակաբույսեր</dt>
                        <dd>{formatCount(cropTypeSummary.class_counts.perennial)}</dd>
                      </div>
                      <div>
                        <dt>Տեսակը չի որոշվել</dt>
                        <dd>
                          {formatCount(
                            cropTypeSummary.class_counts.undetermined,
                          )}
                        </dd>
                      </div>
                    </dl>
                  ...
                  {cropTypeStatus === "ready" && annualCyclesSummary && (
                    <dl className="activity-summary annual-cycle-summary">
                      <div>
                        ...
                        <dd>{formatCount(annualCyclesSummary.class_counts.single_cycle)}</dd>
                      </div>
                      <div>
                        <dt>Միամյա · երկու ցիկլ</dt>
                        <dd>
                          {formatCount(annualCyclesSummary.class_counts.two_cycle_recurring)}
                        </dd>
                      </div>
                    </dl>
                  )}
              ...
              <div className="land-class-list" aria-label={activeLandMode.label}>
              ...
              <div className="land-schema-status" role="status">
                {landMode === "activity_2026"
                  ? "Նախնական դիտարկում․ դասերի սահմանները դեռ հաստատված չեն"
                  : landMode === "land_use_type" && cropTypeStatus === "ready" && activitySummary
                    ? `Տնամերձ գյուղատնտեսություն՝ ${formatCount(activitySummary.household_parcel_count)} հողամաս · 2021–2025 EO դասակարգումը դեռ ենթակա է տեսողական հաստատման`
                    : landMode === "history_2021_2025" && historyStatus === "ready" && historySummary
                      ? `EO պատմությունը դեռ հաշվարկված չէ ${formatCount(historySummary.class_counts.not_calculated)} հողամասի համար · լրացուցիչ ստուգում՝ ${formatCount(historySummary.class_counts.insufficient)}`

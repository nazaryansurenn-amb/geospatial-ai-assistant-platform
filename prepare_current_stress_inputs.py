...
VERSION='current_stress_inputs_20260907_v1'
    ...
    with store.lock():
        ...
        if not manifest.exists():
            ...
            for collection in config['eo_collections']:
                ...
                page=request(session,'POST',SEARCH,json=body).json()
                assert not any(v.get('rel')=='next' for v in page.get('links',[])),'Narrow query requires pagination'
                for scene in page['features']:

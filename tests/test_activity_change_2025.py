...
import pytest
from wp_core.activity_change_2025 import classify, CLASSES

ROOT = Path(__file__).resolve().parents[1]
REVIEW = ROOT / "server_data/review/activity_change_2025_review_20260906_v1"


@pytest.mark.parametrize("codes, expected", [
    ([3,3,3,3,1], "became_active"), ([3,3,3,3,2], "became_active"),
    ([1,1,1,1,3], "became_inactive"), ([2,1,2,1,3], "became_inactive"),
    ([1,3,3,3,3], "not_selected"),  # Stopped in 2022, not 2025.
...
def test_all_possible_five_year_patterns():
    for values in itertools.product(range(4), repeat=5):
        ...
        if values[:4] == (3,)*4 and values[4] in (1,2):
            expected = "became_active"
        if set(values[:4]) <= {1,2} and values[4] == 3:
            expected = "became_inactive"
...
def test_agent_context_addition_preserves_other_modes(monkeypatch):
    from wp_core.activity_change_agent import install, MODE
    from wp_core import agent_service_v4 as service, agent_presentation
    ...
    assert "Cultivation changes in 2025" in text
...
def test_ui_has_sixth_tab_and_vector_only_delivery():
    ...
    assert schema.index('id: "activity_change_2025"') > schema.index('id: "consolidation"')
...
def test_register_covers_all_immutable_parcels():
    ...
    path = ROOT / "data/analysis/activity_change/activity_change_2025_review_20260906_v1"

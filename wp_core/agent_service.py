...
from wp_core.agent_knowledge import instructions
...
def openai_response(messages):
    ...
    payload={'model':model,'store':False,'instructions':instructions(), 'input':messages,
             'tools':TOOLS,'parallel_tool_calls':False,'max_output_tokens':2400,'reasoning':{'effort':'none'}}
...
class AgentService:
    ...
    def _run(self,token,jid,text,context):
        ...
        try:
            ...
            current=[{'role':'user','content':json.dumps({'question':text,'current_map_context':context},ensure_ascii=False)}]
            messages+=current

...
from wp_core.agent_knowledge_v7 import instructions
from wp_core.agent_excel_v7 import result_workbook
from wp_core.agent_presentation_v7 import reader_context, presentation_issues, rewrite_request
from wp_core.agent_scope import KINDS, REPLY_FORMAT, direct_kind, fixed_reply
from wp_core.agent_queries_v7 import CLASSES, TOPICS, SCOPES, COMMUNITY_NAMES, QueryError, build_result, parcel_result, persist_result
...
QUERY_PROPERTIES = {
    ...
    'years': {'type':'array','items':{'type':'integer'},'description':'Activity and irrigation stress [2026]; history subset 2021-2025; other topics full 2021-2025. [] uses default.'},
}
...
TOOLS = [
    ...
    {'type':'function','name':'analyze_land','description':'Calculate counts, official hectares, measured active hectares, existing class and year/community breakdowns. Returns an attached table, chart, Excel download and map-selection card.',
...
def clean_context(value):
    ...
    if mode not in ['activity_2026','history_2021_2025','land_use_type','potential','consolidation','degradation','irrigation_stress']:
...
class AgentService:
    ...
    def result_path(self,token,rid,kind):
        ...
        return ROOT/'server_data/agent_v7/results'/rid/name
    ...
    def _run(self,token,jid,text,context):
        ...
        try:
            ...
            messages+=current
            direct=direct_kind(text)
            if direct:
                answer=fixed_reply(direct,context['language'])
                current.append({'role':'assistant','content':json.dumps({'kind':direct,'answer':answer},ensure_ascii=False)})
                with self.lock:
            ...
            for round_number in range(8):
                ...
                if not calls:
                    ...
                    try:
                        ...
                        if not isinstance(reply,dict) or set(reply)!={'kind','answer'} or reply['kind'] not in KINDS or not isinstance(reply['answer'],str):raise ValueError()
                    except (ValueError,TypeError):
                        raise ServiceError('The assistant could not prepare a valid reply. Please try again.') from None
                    answer=reply['answer'] if reply['kind']=='domain' else fixed_reply(reply['kind'],context['language'])
                    ...
                    if presentation_issues(answer,context['language']):
                        ...
                        correction={'role':'developer','content':rewrite_request(context['language'])+' Preserve the platform_reply JSON format and the conversation boundary.'}
        ...
        def do_GET(self):
            ...
            try:
                ...
                if len(parts)==4 and parts[2]=='job':
                    ...
                    excel=parts[4]=='xlsx'
                    file=service.result_path(self._token(),parts[3],'summary' if excel else parts[4])
                    data=result_workbook(file.parent) if excel else file.read_bytes()
                    self.send_response(200)
                    self.send_header('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' if excel else 'text/csv; charset=utf-8' if parts[4]=='csv' else 'application/json; charset=utf-8')
                    if excel:self.send_header('Content-Disposition','attachment; filename="land-analysis.xlsx"')

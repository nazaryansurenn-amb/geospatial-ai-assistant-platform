...
from wp_core.agent_queries_v7 import label
...
TOPICS={'irrigation_stress':'Ոռոգման սթրեսի հնարավոր նշաններ','activity':'2026 ակտիվություն','history':'Պատմություն՝ ըստ տարիների','history_summary':'2021–2025 պատմություն','use_type':'Օգտագործման տեսակ','cycles':'Տարեկան ցիկլեր','potential':'Ներուժ','consolidation':'Կոնսոլիդացիայի հնարավորություն','parcel':'Հողամաս','degradation':'Հողերի դեգրադացիայի հնարավոր նշաններ'}
...
def workbook_bytes(result,parcel_csv):
    ...
    if q['topic']=='degradation':summary.extend([['Մեկնաբանություն','Հնարավոր նշաններ․ հողի դեգրադացիան հաստատված չէ։ Հեկտարները վնասված հատվածի չափում չեն։'],['Գնահատման շրջանակում չգնահատված հողամաս',s.get('assessment_unassessed_parcels')],['Չգնահատված կադաստրային հա',s.get('assessment_unassessed_official_area_ha')]])
    if q['topic']=='irrigation_stress':summary.extend([['Մեկնաբանություն','Ոռոգման սթրեսի հնարավոր նշաններ․ անհրաժեշտ է տեղում ստուգում։ Հեկտարները սթրեսի ենթարկված հատվածի չափում չեն։ Հարևան հողամասերը կարող են կիսել արբանյակային ազդանշանը։'],['Եղանակ','ECMWF եղանակային մոդելի գնահատականներ'],['Չգնահատված հողամաս',s.get('assessment_unassessed_parcels')],['Չգնահատված կադաստրային հա',s.get('assessment_unassessed_official_area_ha')]])
    ...
    for r in csv.DictReader(StringIO(parcel_csv)):
        parcels.append([r['cadastre_code'],label(r['community'],'hy'),number(r['official_area_ha']),label(r['activity_stage'],'hy'),
            ...
            label(r['class'],'hy'),number(r.get('year'),True),(r.get('stress_observation_date') if q['topic']=='irrigation_stress' else number(r.get('observed_active_area_ha')))])

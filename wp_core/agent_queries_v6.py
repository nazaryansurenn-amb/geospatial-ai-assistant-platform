...
RESULTS = ROOT / 'server_data/agent_v6/results'
DATE = '2026-08-23'
TOPICS = ['activity', 'history', 'history_summary', 'use_type', 'cycles', 'potential', 'consolidation', 'degradation']
...
def validate(args, context=None):
    ...
    if q['topic'] in ['potential', 'consolidation', 'degradation'] and q['households'] != 'exclude':
    ...
    if not classes and q['topic'] in ['potential', 'consolidation', 'degradation']:
        classes = ['gravity_candidate', 'mechanical_candidate'] if q['topic'] == 'potential' else (['possible_signs'] if q['topic']=='degradation' else ['candidate'])

...
RESULTS = ROOT / 'server_data/agent_v1/results'
DATE = '2026-08-23'
TOPICS = ['activity', 'history', 'history_summary', 'use_type', 'cycles', 'potential', 'consolidation']
...
def validate(args, context=None):
    ...
    if q['topic'] in ['potential', 'consolidation'] and q['households'] != 'exclude':
    ...
    if not classes and q['topic'] in ['potential', 'consolidation']:
        classes = ['gravity_candidate', 'mechanical_candidate'] if q['topic'] == 'potential' else ['candidate']
    ...
    if q['topic'] != 'activity' and any(y not in range(2021,2026) for y in years):
        raise QueryError('Completed-season analyses use 2021-2025 only.')
    if q['topic'] in ['history_summary','use_type','cycles','potential','consolidation'] and sorted(set(years)) != list(range(2021,2026)):
...
def build_result(args, context=None, language='en', index=INDEX):
    ...
    return {'query':q,'summary':summary,'rows':rows,'notes':notes,'observation_date':DATE if topic=='activity' else None,

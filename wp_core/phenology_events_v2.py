...
def classify_year(
    ...
    result: dict[str, Any] = {
        ...
        "soil_interval_method": "continuous_low_envelope_with_repeated_soil_witnesses",
    ...
    low_vegetation = (ndvi <= policy.soil_ndvi_maximum) & (evi2 <= policy.soil_evi2_maximum)
    soil = low_vegetation & (bsi > policy.soil_bsi_minimum)
    ...
    for low_run in _runs(low_vegetation, day, min(policy.maximum_soil_step_days, policy.maximum_event_gap_days)):
        # A nonpositive BSI does not by itself show vegetation recovery. Retain
        # those actual low observations inside the envelope without calling
        # them soil. Either vegetation index above its low limit ends the run.
        soil_groups: list[list[int]] = []
        for i in low_run:
            if not soil[i]:
                continue
            if soil_groups and day[i] - day[soil_groups[-1][-1]] <= policy.maximum_soil_step_days:
                soil_groups[-1].append(i)
            else:
                soil_groups.append([i])
        for soil_witnesses in soil_groups:
            if (len(soil_witnesses) < policy.minimum_soil_dates
                    or day[soil_witnesses[-1]] - day[soil_witnesses[0]] < policy.minimum_soil_span_days):
                continue
            # Only the first/last positive-BSI witnesses bound the separator.
            # All intervening low dates must cover the same comparable ground.
            run = list(range(soil_witnesses[0], soil_witnesses[-1] + 1))
    ...
    for segment in segments:
        ...
        end_witnesses = [] if not following else [i for i in trailing if i in following and soil[i]
        ...
        events.append({
            ...
            "soil_witness_count": sum(bool(soil[i]) for i in following) if following else 0,
            "soil_intermediate_low_dates": sum(not bool(soil[i]) for i in following) if following else 0,

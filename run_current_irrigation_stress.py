...
CONFIG=ROOT/'config/irrigation_stress_20260907_v1.json';CFG=json.loads(CONFIG.read_text())
...
def main():
    ...
    with run_lock(OUT/'run.lock'):
        ...
        with np.load(BASE/'sampling/exact_area_20m.npz') as w:
            total=np.bincount(w['parcel'],weights=w['area'],minlength=len(sampling));pure=w['area']>=360
            number=np.bincount(w['parcel'][pure],minlength=len(sampling));area=np.bincount(w['parcel'][pure],weights=w['area'][pure],minlength=len(sampling))
        sampling['spatial_ok']=(number>=CFG['quality']['minimum_full_pixels_20m'])&(area/total>=CFG['quality']['minimum_full_area_fraction'])

"""Independent checks for the private parcel list and precipitation correction."""
import json
...
def main():
    ...
    for year in prepared["config"]["years"]:
        ...
        for cell, g in corrected.groupby(["latitude", "longitude"]):
            ...
            np.testing.assert_allclose(g.rain_mm, expected, atol=1e-12, rtol=1e-9, equal_nan=True)
        diag = pd.read_parquet(folder / "comparisons.parquet")
        nominal = diag.loc[diag.rain_limit_mm.eq(1.) & diag.excursion_db.eq(2.)]
        positive = nominal.loc[nominal.relative_excursion.fillna(False)]
        ...
        sample = pd.concat([nominal.iloc[np.linspace(0, len(nominal)-1, min(80, len(nominal))).astype(int)], positive]).drop_duplicates(["internal_parcel_id", "middle_item"])
        for row in sample.itertuples():
            ...
            for start,end in ((dates[0],dates[1]),(dates[1],dates[2]),(dates[1]-pd.Timedelta(hours=48),dates[1])):
                ...
                checked_windows += 1
            np.testing.assert_allclose(totals,[row.rain_before_mm,row.rain_after_mm,row.rain_recent_48h_mm],atol=1e-10,rtol=1e-9,equal_nan=True)
            ...
            if bool(row.relative_excursion) if pd.notna(row.relative_excursion) else False:
                assert row.supported and row.peer_count >= cfg["minimum_peers"] and max(totals) <= 1.
                assert row.rise_db >= 2. and row.fall_db >= 2.
                ...
                sensitivity.append({"year":year,"internal_parcel_id":row.internal_parcel_id,"middle_item":row.middle_item,
                                    ...
                                    "survives_plus_tolerance_per_hour":all(total+n*tolerance <= 1. for total,n in zip(totals,lengths))})
    ...
    output = {"status":"parcel_delivery_independently_verified","checked_parcels":len(result),"checked_rainfall_windows":checked_windows,
              "nominal_inspection_parcels":len(shortlist),"nominal_observations_sensitivity":sensitivity,
              "all_nominal_observations_stable_to_precision_checks":all(x["survives_half_tolerance"] and x["survives_double_tolerance"] and x["survives_plus_tolerance_per_hour"] for x in sensitivity),
              "checker_sha256":m.peer.sha(Path(__file__)),"result_manifest_sha256":m.peer.sha(m.OUT / "complete.json")}
    dest = m.ROOT / "server_data/review" / m.VERSION / "verification.json"

...
export const POTENTIAL_LABELS = {
  gravity_candidate: "Ինքնահոս ոռոգման նախնական թեկնածուներ",
  mechanical_candidate: "Մեխանիկական ոռոգման նախնական թեկնածուներ",
  review: "Լրացուցիչ ստուգման ենթակա",
  excluded: "Բացառված է ներուժի գնահատումից",
  not_candidate: "Ներուժի թեկնածու չի առանձնացվել",
};
  ...
  useEffect(() => {
    ...
    Object.entries(COLORS).forEach(([kind, color]) => {
      for (const type of ["fill", "line"]) {
        const id = `land-potential-${kind}-${type}`;
        if (!map.getLayer(id)) map.addLayer({id, type, source:"land-analytics", "source-layer":"land_analytics", minzoom:10,
          layout:{visibility:"none"}, paint:type === "fill" ? {"fill-color":color,"fill-opacity":0} : {"line-color":color,"line-width":1.3,"line-opacity":0}}, "cadastre-hit");
        const filter = ["all", ["==", ["get", "potential_class"], kind]];
        if (scope === "stage_1" || scope === "stage_2") filter.push(["==", ["get", "stage"], scope]);
        const show = active && scope !== "wua" && visibility[kind];
        map.setFilter(id, filter);
        map.setLayoutProperty(id, "visibility", show ? "visible" : "none");
        map.setPaintProperty(id, `${type}-opacity`, show ? (type === "fill" ? opacity/100 : .95) : 0);
      }
    });
  }, [map, ready, payload, scope, active, visibility, opacity]);
  if (!active) return null;
  const summary = payload?.summaries?.[scope];
  return <div className="activity-preview" aria-label="Ներուժի նախնական գնահատում">
    <label className="opacity-control opacity-control--activity"><span>Թափանցիկություն</span><output>{opacity}%</output>
      <input aria-label="Ներուժի թափանցիկություն" type="range" min="20" max="85" step="5" value={opacity} onChange={(e)=>setOpacity(Number(e.target.value))}/>
    </label>
    {!summary ? <p>Տվյալները բեռնվում են…</p> : <>
      ...
      <div className="land-class-list" aria-label="Ներուժի դասեր">
        {Object.entries(COLORS).map(([kind,color])=><label key={kind} className="layer-option layer-option--land">
          <input type="checkbox" checked={visibility[kind]} onChange={(e)=>setVisibility({...visibility,[kind]:e.target.checked})}/>
          <span className="land-class-swatch" style={{backgroundColor:color}} aria-hidden="true"/>
          <span>{POTENTIAL_LABELS[kind]} · {count(summary.class_counts[kind])}</span>
        </label>)}
      </div>
      <p className="activity-preview-status">Ստորին Հրազդան I + II սահմաններում։ Մերձակա տարածքները դեռ չեն գնահատվել։</p>
    </>}
  </div>;
}

"""Derive strict changes, validate all yearly inputs, and prepare a new local UI."""
import json
...
from wp_core.activity_change_2025 import YEARS, CLASSES, classify, summary

SLUG = "activity_change_2025_review_20260906_v1"
BASE_SLUG = "agent_review_20260906_v2"
...
BASE_LOCKS = [f"agent_review_20260906_v{i}" for i in range(1, 5)] + ["hectares_review_20260906_v1", "consolidation_review_20260906_v2"]
...
def prepare():
    ...
    frame["change_class"] = [classify(r.annual_state_codes, r.profile_year_count, bool(r.household), bool(r.road_excluded)) for r in frame.itertuples()]
    ...
    lookup = {r.cadastre_code: {"changeClass": r.change_class, "year": 2025} for r in frame.itertuples()}
    payload = {"analysis_version": SLUG, "observation_years": list(YEARS), "change_year": 2025,
    ...
    payload["tile_delivery"] = {"url": f"/{PUBLIC_DIR}/{{z}}/{{x}}/{{y}}.pbf", "layer_name": "activity_change", "min_zoom": 8, "max_zoom": 15,
                                "bounds": list(basis.total_bounds)}
    ...
    write_json(OUT / "manifest.json", {"analysis_version": SLUG, "created_at": payload["created_at"],
        ...
        "rule": "Four consistent adequately observed baseline seasons; opposite state in 2025; active includes partial",
...
def finish():
    ...
    files = [ROOT / "run_activity_change_2025_review.py", ROOT / "wp_core/activity_change_2025.py", ROOT / "wp_core/activity_change_agent.py", ROOT / "scripts/prepare_activity_change_2025.py",
    ...
    write_json(LOCK, {"version": SLUG, "base": "agent_review_20260906_v4", "port": 8526,
    ...
    print("Verified and sealed strict 2025 changes")
